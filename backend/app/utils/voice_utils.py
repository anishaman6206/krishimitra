...
async def opus_to_wav(input_path: str, output_path: str) -> None:
    """Convert OGG/Opus to WAV/PCM 16kHz mono using ffmpeg (non-blocking)."""
    ...
    if proc.returncode != 0:
        raise RuntimeError("ffmpeg failed converting opus->wav")
...
async def wav_to_opus(input_path: str, output_path: str) -> None:
    ...
    await proc.wait()
    if proc.returncode != 0:
        raise RuntimeError("ffmpeg failed converting wav->opus")
